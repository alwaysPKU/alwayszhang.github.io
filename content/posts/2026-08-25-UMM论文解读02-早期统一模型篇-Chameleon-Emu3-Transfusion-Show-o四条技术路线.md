...
### Show-o2：向视频统一迈进（2025）

Show-o 的后续版本 **Show-o2** 把"单 Transformer 统一离散理解 + 连续生成"从图像推进到视频，核心升级是引入 **3D Causal VAE** 做时空压缩，把视频编码成带因果约束的 token，并设计统一的 **Flow Head** 同时支撑图像与视频生成。它在一个模型内覆盖文本理解、图像生成、视频生成，是早期把 AR + 扩散混合范式系统化扩展到时空维度的代表性开源工作。Show-o2 的 7B 版本也因此成为 Part 5 中 Lance 视频理解（MVBench）对比的重要基线。需要注意的是，它仍是"理解走离散 AR、生成走连续扩散"的双路径设计，与 BAGEL/Lance 的 MoT 双塔、U1 的原生统一属于不同路线。
