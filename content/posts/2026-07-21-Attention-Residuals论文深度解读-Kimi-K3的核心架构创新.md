...
$$\alpha_{i \to l} = \varphi(w_l, k_i) / \sum \varphi(w_l, k_j)$$
$$\varphi(q, k) = \exp(q^T \cdot \text{RMSNorm}(k))$$
...
$$\alpha_{i \to l} = \exp(w_l^T \cdot \text{RMSNorm}(k_i)) / \sum_j \exp(w_l^T \cdot \text{RMSNorm}(k_j))$$

### A.3 Block AttnRes
块内：$b_n = \sum_{j \in B_n} f_j(h_j)$

块间：$h_l = \sum_{n=0}^{N-1} \alpha_{n \to l} \cdot b_n + \alpha_{n' \to l} \cdot b_{n'}^{i-1}$
